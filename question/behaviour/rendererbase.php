...
abstract class qbehaviour_renderer extends plugin_renderer_base {
    ...
    /**
     * Generates a button for a @see question_attempt to trigger the replay process.
     *
    ...
    protected function replay_button(question_attempt $qa, question_display_options $options) {
        if (!$qa->get_state()->is_active() || !property_exists($options, 'replayresponse') || $options->replayresponse != 1) {
            return '';
        }
        $attributes = array(
            'type' => 'button',
        ...
        $output = html_writer::empty_tag('input', $attributes);
        if (!$options->readonly) {
            $this->page->requires->js_call_amd('core_question/replay', 'init', array($attributes['id'], $qa->get_database_id(), $qa->get_behaviour_field_name('replaysequence')));
        }
        return $output;
    }
